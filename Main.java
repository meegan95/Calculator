...
public class Main {
    ...
    static int calc1 (int num1, int num2, String input) {
       int r=0;
       if (num1==0 || num2==0 || num1>10 || num2>10){System.exit(1);}
        if (input.contains("+")) {r=num1+num2;}
        if (input.contains("-")) {r=num1-num2;}
        if (input.contains("/")) {r=num1/num2;}
        if (input.contains("*")) {r=num1*num2;}return r;}
    public static String calc(String input) {
        ...
        if (one.codePointAt(0) >= 73 && two.codePointAt(0) >= 73) {
            cifra n1 = Main.cifra.valueOf(one);
            cifra n2 = Main.cifra.valueOf(two);
            int num1= n1.ordinal();
            int num2= n2.ordinal();
            int result = calc1(num1, num2, input);
            if(result>0){cifra[] num3 = cifra.values();
            cifra ress = num3[result];
